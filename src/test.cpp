...
void mcCb(const geometry_msgs::PoseStamped::ConstPtr &msg) {
  ...
  tf::Quaternion qin(msg->pose.orientation.x, msg->pose.orientation.y,
                     msg->pose.orientation.z, msg->pose.orientation.w);

  tf::Quaternion qout(out.pose.orientation.x, out.pose.orientation.y,
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber tf_sub =
      nh.subscribe<geometry_msgs::PoseStamped>("/test/mocap", 10, mcCb);
  ros::Publisher tf_pub =
      nh.advertise<geometry_msgs::PoseStamped>("test/trans", 10);
  ros::Publisher ri_pub = nh.advertise<geometry_msgs::Point>("test/rpyin", 10);
  ros::Publisher ro_pub = nh.advertise<geometry_msgs::Point>("test/rpyout", 10);
  tf_ = &tf_pub;
  rpyin_ = &ri_pub;
  rpyout_ = &ro_pub;
  ros::Rate rate(100);
