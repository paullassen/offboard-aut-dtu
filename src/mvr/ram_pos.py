...
class fcuModes:
    ...
    def setTakeoff(self):
    	rospy.wait_for_service('ram_elsa/mavros/cmd/takeoff')
    	try:
    		takeoffService = rospy.ServiceProxy('ram_elsa/mavros/cmd/takeoff', mavros_msgs.srv.CommandTOL)
    		takeoffService(altitude = 3)
    	except rospy.ServiceException, e:
    		print "Service takeoff call failed: %s"%e

    def setArm(self):
        rospy.wait_for_service('ram_elsa/mavros/cmd/arming')
        try:
            armService = rospy.ServiceProxy('ram_elsa/mavros/cmd/arming', mavros_msgs.srv.CommandBool)
            armService(True)
        except rospy.ServiceException, e:
    ...
    def setStabilizedMode(self):
        rospy.wait_for_service('ram_elsa/mavros/set_mode')
        try:
            flightModeService = rospy.ServiceProxy('ram_elsa/mavros/set_mode', mavros_msgs.srv.SetMode)
...
class Controller():
    def __init__(self):
        ...
        self.att_publisher = rospy.Publisher('ram_else/mavros/setpoint_raw/attitude', AttitudeTarget, queue_size=1)
...
def main():
    ...
    rospy.Subscriber('ram_elsa/mavros/state', State, cnt.stateCb)
