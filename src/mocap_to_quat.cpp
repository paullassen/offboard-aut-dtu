...
void write_csv(std::string filename, std::vector<std::pair<std::string, std::vector<long double> > > dataset){
    ...
    for(int i = 0; i < dataset.at(0).second.size(); ++i)
    {
		...
		int l = k+1;
		int m = l+1;
		tf::Quaternion q(dataset.at(j).second.at(i),
						 dataset.at(k).second.at(i),
						 dataset.at(l).second.at(i),
						 dataset.at(m).second.at(i));
		tf::Matrix3x3 mat(q);
...
int main(int argc, char ** argv) {
	...
	std::string path = "/home/pala/Documents/MATLAB/bagger/test17/bagfile-_test_mocap.csv";
    // Read three_cols.csv and ones.csv
    std::vector<std::pair<std::string, std::vector<long double> > > rpy = read_csv(in.str());
