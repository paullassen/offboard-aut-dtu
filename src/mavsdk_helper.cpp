...
inline void action_error_exit(Action::Result result,
  ...
  if (result != Action::Result::Success) {
    std::cerr << ERROR_CONSOLE_TEXT << message << result << NORMAL_CONSOLE_TEXT
              << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
Telemetry::LandedStateCallback landed_state_callback(
    std::shared_ptr<Telemetry>& telemetry, std::promise<void>& landed_promise) {
...
int mavmain(int argc, char** argv) {
  Mavsdk dc;
  ...
  telemetry->subscribe_landed_state(
      landed_state_callback(telemetry, in_air_promise));
  in_air_future.wait();
